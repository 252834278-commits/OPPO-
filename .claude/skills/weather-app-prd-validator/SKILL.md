...
- **预警类型/等级/颜色枚举**（必须引用权威枚举或 `reference.md`）
...
需要引用模版结构与口径时，优先阅读本目录下的：
- `reference.md` - 口径与资料索引
- `examples.md` - 输入输出风格示例
...
#### 桌面组件/负一屏：刷新与展示规则
- 组件尺寸：支持的尺寸规格与信息密度
- 刷新机制：系统调度/手动刷新/最小间隔与频率限制
- 展示内容：核心字段（温度/天气图标/位置）与扩展字段（预警/AQI）
- 点击行为：跳转目标（首页/详情/设置）与深链参数
- 主题适配：深色模式/动态主题/壁纸适配策略
- 失败兜底：数据过期/获取失败时的展示与提示
```
...
```markdown
#### 埋点需求：事件定义表
| 事件名 | 触发时机 | 事件属性 | 属性口径/取值 | 采样率 | 备注 |
|---|---|---|---|---|---|
| weather_page_view | 进入天气首页 | city_id, source, is_located | city_id: 城市编码; source: push/icon/widget; is_located: bool | 100% | 用于统计 DAU |
| weather_refresh | 下拉刷新完成 | city_id, result, latency_ms | result: success/fail/timeout; latency_ms: 请求耗时 | 100% | 用于性能监控 |
| alert_card_click | 点击预警卡片 | alert_type, alert_level, city_id | alert_type: 预警类型枚举; alert_level: 颜色枚举 | 100% | 用于预警关注度分析 |
```

## 评分卡口径说明（0-5 分）

统一用 0–5 分，口径如下（保持自洽）：
- **5**：信息完整且可直接开发/测试，几乎无 P0/P1
- **4**：少量 P1，边界与验收基本可落地
- **3**：有明显缺口（多处待确认），需要补齐后再开工
- **2**：关键流程/口径/合规缺失，风险高
- **1**：只有愿景/概念描述，几乎不可执行
- **0**：与需求无关或信息严重不足
...
### 就绪度判定
- **Ready**：P0 = 0 且 P1 ≤ 2，评分卡各维度 ≥ 4
- **Needs work**：P0 = 0 且 P1 ≥ 3，或评分卡有维度 < 4 但 ≥ 2
- **Not ready**：P0 ≥ 1，或评分卡有维度 < 2

### 总体风险判定
- **低**：可直接进入开发，无明显返工风险
- **中**：需补充部分信息，但核心流程清晰
- **高**：关键信息缺失，开发/测试无法启动，合规风险高
