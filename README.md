...
└── .claude/                    # Claude Code 配置目录 ⭐
    └── skills/                 # 技能文档目录（Claude Code 标准位置）
        ...
        │   └── SKILL.md
        └── weather-app-prd-validator/           # 天气 PRD 校验 ⭐ NEW
            ├── SKILL.md
            ├── reference.md
            └── examples.md
```
...
```
weather-app-prd-validator/
├── SKILL.md           # 主指令文档（必需）
├── reference.md       # 口径与资料索引（可选）
├── examples.md        # 示例输出（可选）
└── scripts/           # 辅助脚本（可选）
```
...
### 3. 天气 App PRD 校验 (weather-app-prd-validator) ⭐ NEW
...
**触发方式**:
- 自动触发：当你提到"校验"、"评审"、"验收标准"、"边界条件"、"埋点"、"预警"、"生活指数"等关键词
- 手动调用：`/weather-app-prd-validator`

**核心能力**:
- **模板硬校验**：对照天气 PRD 模版做 13 项 Gate 检查（P0/P1 阻塞项）
- **天气专项校验**：7 大维度深度校验
  - A. 天气数据源与口径（含降雨提醒）
  - B. 定位与多城市管理
  - C. 预警与通知
  ...
  - F. 非功能与工程可行性
  - G. 合规与安全
- **可测试性检查**：识别"更好/更准/更流畅"等不可测表述并提供改写建议
- **可复制改写**：输出可直接落回 PRD 的文案模板

**输出结构**:
1. PRD 校验结论（就绪度/阻塞项/风险等级）
2. 章节覆盖率表格
3. 评分卡（8 维度 0-5 分）
4. 待确认信息清单
5. 逐章点评
...
**配套文件**:
- `reference.md`：口径与资料索引（预警枚举、生活指数口径、AQI 标准等）
- `examples.md`：输入→输出风格示例

**关键约束**:
- **不编造口径**：预警类型、生活指数阈值、性能指标等缺失时写"待确认"
- **防拍脑袋**：所有建议基于模板规范和天气领域最佳实践
...
- **2026-02-05**: 添加天气 App PRD 校验技能（weather-app-prd-validator）
